...
from .entity_alignment import Entity, align_entities, entity_record, labels_to_entities


SPLITS = ("validation", "test")
COVERAGE_LEVELS = {"insufficient": 0, "limited": 1, "adequate": 2}
COVERAGE_NAMES = {value: key for key, value in COVERAGE_LEVELS.items()}
ENTITY_PREDICTION_OUTCOMES = (
    "correct",
    "wrong_label",
    "span_mismatch",
    "wrong_label_and_span",
    "complex_overlap",
    "missed",
    "extra",
)
...
def entity_fields(entities: tuple[Entity, ...], row: dict[str, Any], field: str) -> str:
    values = []
    for entity in entities:
        record = entity_record(entity, row) or {}
        value = record.get(field)
        values.append("" if value is None else str(value))
    return "; ".join(values)


def entity_span_fields(entities: tuple[Entity, ...], index: int) -> str:
    return "; ".join(str(entity[index]) for entity in entities)


def entity_prediction_rows(
    *,
    split: str,
    ...
    for prediction in prediction_rows:
        ...
        gold_entities = labels_to_entities([str(label) for label in prediction["gold_labels"]])
        pred_entities = labels_to_entities(
            [str(label) for label in prediction["pred_labels"]],
            merge_adjacent_same_label=True,
        )
        for alignment in align_entities(gold_entities, pred_entities):
            rows.append(
                {
                    "split": split,
                    "document_id": doc_id,
                    "language": str(source.get("language") or prediction.get("language") or ""),
                    "date": str(source.get("date") or prediction.get("date") or ""),
                    "outcome": alignment.outcome,
                    "gold_start": entity_span_fields(alignment.gold, 0),
                    "gold_stop": entity_span_fields(alignment.gold, 1),
                    "gold_surface": entity_fields(alignment.gold, source, "surface"),
                    "gold_label": entity_span_fields(alignment.gold, 2),
                    "prediction_start": entity_span_fields(alignment.predictions, 0),
                    "prediction_stop": entity_span_fields(alignment.predictions, 1),
                    "prediction_surface": entity_fields(alignment.predictions, source, "surface"),
                    "prediction_label": entity_span_fields(alignment.predictions, 2),
                }
            )
    return sorted(rows, key=lambda row: (row["split"], row["document_id"], int(row["gold_start"].split("; ", 1)[0] or row["prediction_start"].split("; ", 1)[0] or 0), row["outcome"]))


def entity_prediction_summary(rows: list[dict[str, str]]) -> dict[str, Any]:
    by_outcome = {outcome: 0 for outcome in ENTITY_PREDICTION_OUTCOMES}
    by_label: dict[str, dict[str, int]] = {}
    for row in rows:
        outcome = row["outcome"]
        by_outcome[outcome] = by_outcome.get(outcome, 0) + 1
        labels = [label for label in row["gold_label"].split("; ") if label]
        for label in labels:
            label_counts = by_label.setdefault(label, {outcome_name: 0 for outcome_name in ENTITY_PREDICTION_OUTCOMES if outcome_name != "extra"})
            label_counts[outcome] = label_counts.get(outcome, 0) + 1
    return {"by_outcome": by_outcome, "by_gold_label": dict(sorted(by_label.items()))}
...
def write_entity_prediction_tsv(path: Path, rows: list[dict[str, str]]) -> None:
    columns = [
        "split",
        "document_id",
        "language",
...
def render_report(
    ...
    *,
    release: str,
    model: str,
    entity_predictions_tsv: str | None = None,
    entity_predictions_summary: dict[str, Any] | None = None,
) -> str:
    lines = [
    ...
    if entity_predictions_summary is not None:
        by_outcome = entity_predictions_summary["by_outcome"]
        lines.extend(
            [
                "",
        ...
        if entity_predictions_tsv:
            lines.extend(["", f"Detailed TSV: `{entity_predictions_tsv}`"])
        lines.extend(["", "| Outcome | Instances |", "|---|---:|"])
        for outcome in ENTITY_PREDICTION_OUTCOMES:
            lines.append(f"| {outcome} | {int(by_outcome.get(outcome, 0))} |")
        ...
        for label, counts in entity_predictions_summary["by_gold_label"].items():
            gold_rows = sum(int(counts.get(outcome, 0)) for outcome in ENTITY_PREDICTION_OUTCOMES if outcome != "extra")
            lines.append(
                f"| `{label}` | {gold_rows} | {int(counts.get('correct', 0))} | {int(counts.get('wrong_label', 0))} | "
                f"{int(counts.get('span_mismatch', 0))} | {int(counts.get('wrong_label_and_span', 0))} | "
                f"{int(counts.get('complex_overlap', 0))} | {int(counts.get('missed', 0))} |"
            )
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--test-entity-predictions-tsv", default="")
    ...
    for split in SPLITS:
        ...
        if split == "test":
            test_entity_predictions = entity_prediction_rows(split=split, source_rows=source_rows, prediction_rows=prediction_rows)

    entity_predictions_tsv = args.test_entity_predictions_tsv or ""
    if entity_predictions_tsv:
        write_entity_prediction_tsv(Path(entity_predictions_tsv), test_entity_predictions)
    prediction_summary = entity_prediction_summary(test_entity_predictions)
    ...
    output.write_text(
        render_report(
            ...
            model=args.model,
            entity_predictions_tsv=entity_predictions_tsv or None,
            entity_predictions_summary=prediction_summary,
        ),
        encoding="utf-8",
    )
    print(
        json.dumps(
            {
                ...
                "status": "current",
                "splits": list(SPLITS),
                "test_entity_predictions_tsv": entity_predictions_tsv,
            },
            sort_keys=True,
        )
    )
