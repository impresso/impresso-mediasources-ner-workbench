...
Use `org.ent.radiostation.china-radio-international` across the historical names of China's international broadcaster: `Radio Peking` / `Radio-Pékin`, `Radio Beijing`, and `China Radio International` / `CRI`. Preserve the visible historical surface and do not confuse the international service with the separate local Radio Beijing International station.
