...
Review includes all unresolved suggestions by default, matching the `to_review` count in `curation-state`. Set `REVIEW_ONLY_UNDER_TARGET=true` only when deliberately restricting a session to label-language buckets that remain below their coverage target.
